...
// encodeMapIntInt encodes map[int]int with ZERO reflection allocations.
//
//go:inline
func (e *Encoder) encodeMapIntInt(mapInterface interface{}, mapLen int) error {
	if err := writeMapHeader(e, 1, mapLen); err != nil {
		return err
	}
	...
	if mapLen >= 50 && e.Buf != nil {
		e.Buf.Grow(mapLen * 16)
	}

	m := mapInterface.(map[int]int)
	for k, v := range m {
		// Encode key
		e.writeFastInt64(int64(k))
...
// writeFastInt64 writes int64 key directly to buffer (no error handling for speed).
//
//go:inline
func (e *Encoder) writeFastInt64(v int64) {
	e.uintScratch[0] = byte(v)
	e.uintScratch[1] = byte(v >> 8)
	e.uintScratch[2] = byte(v >> 16)
	e.uintScratch[3] = byte(v >> 24)
	e.uintScratch[4] = byte(v >> 32)
	e.uintScratch[5] = byte(v >> 40)
	e.uintScratch[6] = byte(v >> 48)
	e.uintScratch[7] = byte(v >> 56)
	e.WriteBytes(e.uintScratch[:8]) // Ignore error (will be caught later)
}
